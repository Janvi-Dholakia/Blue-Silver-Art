...
def generate_bill_number(bill_id):
    ...
    return f"BSA{fy}{month}{bill_id:04d}"
...
def save_bill():
    ...
    cur.execute("""
    INSERT INTO bills
    (customer_name, place, phone, date, total)
    VALUES (?, ?, ?, ?, ?)
    """, (
        data['customer_name'],
        data['place'],
    ...
    bill_no = generate_bill_number(bill_id)
    ...
    for item in data['items']:
        ...
        (bill_id, description, net_weight,
        touch, wastage, labour, silver, amount)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            bill_id,
            item['description'],
